...
#include "rwip_config.h"             // SW configuration
    ...
    #if (DEBUG_WRITE_TO_SPI && defined(PRODUCTION_DEBUG_OUTPUT))
        #include "app_flash.h"
    #endif
...
void app_init(void)
{
    ...
    #if (DEBUG_WRITE_TO_SPI && defined(PRODUCTION_DEBUG_OUTPUT))
    char msg[APP_FLASH_MSG_SIZE - sizeof(int) + sizeof(char)];
    if (app_spi_flash_read_dbg_msg(msg, strlen(msg)) == APP_DBG_FAULT_NONE) {
        app_dbg_write_msg(msg);
        app_spi_flash_erase_dbg_msg();
    }
    app_dbg_fault_t fault = app_spi_flash_read_dbg_regs();
    if (fault != APP_DBG_FAULT_FLASH && fault != APP_DBG_FAULT_NONE) {
        app_dbg_write_fault(fault);
        app_spi_flash_erase_dbg_regs();
    }
    #endif
}
