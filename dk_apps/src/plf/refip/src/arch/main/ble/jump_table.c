...
#include "nvds.h"
#include "pll_vcocal_lut.h"  // struct LUT_CFG_struct declaration
...
#else
				(const uint32_t*) wlan_coex_lld_evt_init_func,          // 42. lld_evt_init
#endif
...
#if PRODUCTION_TEST
				(const uint32_t*) prodtest_platform_reset_func,         // 54. platform reset
#else
