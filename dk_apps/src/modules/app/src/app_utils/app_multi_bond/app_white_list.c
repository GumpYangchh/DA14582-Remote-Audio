 ...
 ****************************************************************************************
 */
static void send_white_list_mgt_cmd(bool add, uint8_t peer_addr_type, struct bd_addr const *peer_addr)
{
    ...
    if (con_fsm_params.has_white_list) {
        ...
        if (con_fsm_params.has_virtual_white_list) {
            ...
            if (   (ADDR_PUBLIC == peer_addr_type)
                ...
                if ( (peer_addr_type == ADDR_RAND) && ((peer_addr->addr[BD_ADDR_LEN - 1] & 0xC0) == SMPM_ADDR_TYPE_PRIV_RESOLV) ) {
