        ...
        ke_task_id_t const src_id)
{
    ...
    case GAPC_PAIRING_SUCCEED:
        ...
        if (sec_env->auth & GAP_AUTH_BOND) {
            sec_env->peer_addr_type = app_env.peer_addr_type;
            memcpy(sec_env->peer_addr.addr, app_env.peer_addr.addr, BD_ADDR_LEN);
        }
        app_paired_func();
        break;
        ...
        ke_task_id_t const src_id)
{
    ...
    if (!app_validate_encrypt_req_func(param)) {
        ...
        if(((sec_env->auth & GAP_AUTH_BOND) != 0)
            && (memcmp(&(sec_env->rand_nb), &(param->rand_nb), RAND_NB_LEN) == 0)
            && (sec_env->ediv == param->ediv)) {
            cfm->found = true;
            cfm->key_size = sec_env->key_size;
            memcpy(&(cfm->ltk), &(sec_env->ltk), KEY_LEN);
            // update connection auth
            app_connect_confirm(sec_env->auth);
    ...
    if (status != GAPC_ENCRYPT_REQ_IND_HANDLER_STATUS_OK) {
        if (status == GAPC_ENCRYPT_REQ_IND_HANDLER_STATUS_KEY_NOT_FOUND) {
            // An LL_REJECT_IND with reason "Pin or Key missing" will be sent by the stack.
            
            // When the APP_CON_FSM_END_TIMER expires, an L2CAP_SECURITY_REQUEST will be sent if SECURITY_REQUEST_SEND is selected,
