...
uint8_t multi_key_pressed[sizeof(multi_key_combinations)/
                          sizeof(struct multi_key_combinations_t)] __RETAINED;
#endif
...
#if (HAS_AUDIO)
bool user_audio_sw_pressed=false;
bool user_audio_sw_released=false;
#endif

#if (HAS_BMI055)
bool user_motion_keys_pressed=false;
bool user_motion_left_click_pressed=false;
#endif
 ...
 ****************************************************************************************
 */
static inline void systick_start(const int ticks, const int mode)
{
    systick_hit = false;
	...
	SetWord32(0xE000E010, 1|mode);      // enable systick on 1MHz clock (always ON) with interrupt (mode)
}
 ...
 */
 ...
 static void set_row_to_low(int idx)
{
...
static void set_column_to_input_pullup(int idx)
{
...
static void set_row_to_input_highz(int idx)
{
    if (kbd_out_bitmasks[idx]) {
...
static inline int record_key(const uint16 output, const uint16 input, const int pressed)
{
    ...
    ASSERT_WARNING(input<KBD_NR_INPUTS);
    ...
    // exception: 'Fn'+'c' is served asynchronously to make sure the storage memory can be cleared even when we are disconnected
    if (kbd_keymap[kbd_fn_modifier][output][input] == CLRP) {
        if (!pressed) {
            app_alt_pair_clear_all_bond_data();
            app_con_fsm_request_reset_bonding_data(true);
        }
        return 1;   // "Clear storage memory data is not logged into the buffer any more and not reported as KEY_PRESS_EVT
    }
                            
    if (!block_key) {
        if (app_con_fsm_get_state() == IDLE_ST) {
            app_con_fsm_start_advertising();
            if (app_con_fsm_bonding_data_reset_pending()) {
                app_con_fsm_reset_bonding_data();
                app_con_fsm_request_reset_bonding_data(false);
            }
        } else {
 ...
 ****************************************************************************************
 */
void process_multi_key_combinations(const uint16 output, const uint16 input, const int pressed) 
{
#ifdef MULTI_KEY_COMBINATIONS_ON    
    int i, j;
    ...
    for (i=0; i<sizeof(multi_key_combinations)/sizeof(struct multi_key_combinations_t); i++) {
        ...
        if (multi_key_pressed[i] == (1 << MULTI_KEY_NUM_OF_KEYS)-1) {    
            ...
            switch (key_combination.action) {
            case MULTI_KEY_ACTION_CONNECT_TO_HOST1:
                app_con_switch_to_peer(0);
                break;
            case MULTI_KEY_ACTION_CONNECT_TO_HOST2:
                app_con_switch_to_peer(1);
                break;
            case MULTI_KEY_ACTION_CONNECT_TO_HOST3:
                app_con_switch_to_peer(2);
                break;
            case MULTI_KEY_ACTION_BOND_TO_HOST1:
                app_con_fsm_add_host_to_entry(0);
                break;
            case MULTI_KEY_ACTION_BOND_TO_HOST2:
                app_con_fsm_add_host_to_entry(1);
                break;
            case MULTI_KEY_ACTION_BOND_TO_HOST3:
                app_con_fsm_add_host_to_entry(2);
                break;
            case MULTI_KEY_ACTION_CLEAR_BONDING_DATA:
                app_alt_pair_clear_all_bond_data();
                app_con_fsm_request_reset_bonding_data(true);
                break;
            }
            ...
            if (xorword) { // if any key state changed
                ...
                do {
                    ...
                    if (!record_key(i, bit, press)) {
                        ...
                        // if key has been recorded check for specific active key combinations
                        process_multi_key_combinations(i, bit, press);
                    }
        ...
        case 0xF4: // Special function
            {
                ...
                if (byte == 0xF) {                  // Exception: do something proprietary!
                    ...
                    case 1:     // pair to another
                        if (!pressed) {
                            app_con_fsm_request_disconnect(MULTI_BOND_REJECT_LAST);
                        }
                        break;
                    case 2:     // clear pairing data from storage memory - handled elsewhere now!
                        ASSERT_WARNING(0);
                        break;
                    ...
                    case 4:
                        if (pressed) {
                            user_audio_sw_pressed = true;
                            user_audio_sw_released = false;
                        } else {
                            user_audio_sw_pressed = false;
                            user_audio_sw_released = true;
                        }
                        break;
#endif
#if (HAS_BMI055)
                    ...
                    case 6:
                        if (pressed) {
                            user_motion_left_click_pressed=true;
                        } else {
                            user_motion_left_click_pressed=false;
                        }
                        break;
#endif
                    case 0xF:   // ignore 
...
static int send_hid_report(void)
    ...
    do {
        ...
        // Also send the key report in HID vendor page for streaming demo
#if (HAS_AUDIO || HAS_MOTION)                    
        app_stream_send_keyreport(req);
#endif                    
        ke_msg_send(req);
...
{
#if defined(REMOTE_WAKEUP_ON) 
// "Remote Host may not handle properly RemoteWakeup when the Inactivity Timeout is on (Some Hosts do not expect to receive LL_TERMINATE_IND from Wakeup capable devices while they are sleeping...)"    
    ASSERT_WARNING(con_fsm_params.has_inactivity_timeout == false);
#endif
...
// Number of hosts for force-connect is not equal to MAX_BOND_PEER    
...
void app_hid_create_db(void)
{
...
#if (HAS_BMI055)
    features->report_nb          = 9;
#elif (HAS_AUDIO) 
    features->report_nb          = 8; 
#else    
    ...
    //------STREAM_HANDLE
    features->report_char_cfg[3] = HOGPD_CFG_REPORT_OUT;
    features->report_char_cfg[4] = HOGPD_CFG_REPORT_IN | HOGPD_REPORT_NTF_CFG_MASK | HOGPD_CFG_REPORT_WR;
    features->report_char_cfg[5] = HOGPD_CFG_REPORT_IN | HOGPD_REPORT_NTF_CFG_MASK | HOGPD_CFG_REPORT_WR;
    features->report_char_cfg[6] = HOGPD_CFG_REPORT_IN | HOGPD_REPORT_NTF_CFG_MASK | HOGPD_CFG_REPORT_WR;
    features->report_char_cfg[7] = HOGPD_CFG_REPORT_IN | HOGPD_REPORT_NTF_CFG_MASK | HOGPD_CFG_REPORT_WR;
#endif
    
#if (HAS_BMI055)
   features->report_char_cfg[8] = HOGPD_CFG_REPORT_IN | HOGPD_REPORT_NTF_CFG_MASK | HOGPD_CFG_REPORT_WR;
#endif
...
void app_keyboard_enable(void)
{
    ...
    req->ntf_cfg[0].report_ntf_en[3] = 1;
    req->ntf_cfg[0].report_ntf_en[4] = 1;
    req->ntf_cfg[0].report_ntf_en[5] = 1;
    req->ntf_cfg[0].report_ntf_en[6] = 1;
    req->ntf_cfg[0].report_ntf_en[7] = 1;
#if (DA14580_RCU || DA14582_RCU)
    req->ntf_cfg[0].report_ntf_en[8] = 1;
#endif

   #ifdef APP_STREAM_HID
    req->ntf_cfg[1].boot_kb_in_report_ntf_en = 0;
    req->ntf_cfg[1].boot_mouse_in_report_ntf_en = 0;
    req->ntf_cfg[1].report_ntf_en[0] = 1;
    req->ntf_cfg[1].report_ntf_en[1] = 1;
    req->ntf_cfg[1].report_ntf_en[2] = 1;
    req->ntf_cfg[1].report_ntf_en[3] = 1;
    req->ntf_cfg[1].report_ntf_en[4] = 1;
    req->ntf_cfg[1].report_ntf_en[5] = 1;
    req->ntf_cfg[1].report_ntf_en[6] = 1;
    req->ntf_cfg[1].report_ntf_en[7] = 1;
    req->ntf_cfg[1].report_ntf_en[8] = 1;
    req->ntf_cfg[1].report_ntf_en[9] = 1;
    req->ntf_cfg[1].report_ntf_en[10] = 1;
#endif
